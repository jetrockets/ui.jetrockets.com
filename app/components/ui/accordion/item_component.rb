class Ui::Accordion::ItemComponent < ApplicationComponent
  ...
  erb_template <<~ERB
    <button type="button" class="accordion__header" data-accordion-target="trigger" data-accordion-id="<%= @id %>" aria-expanded="<%= @opened %>">
      ...
      <svg data-accordion-icon class="w-3 h-3 transition-transform duration-300 shrink-0" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
